...
{
	static int iItemFlags, iUserID, iClient;
...
void SurvivorBotThink(int iClient, int &iButtons, int iWpnSlots[6])
{
	...
	if (IsEntityExists(iInfectedTarget))
	{
		...
		if ((g_iCvar_AutoShove_Enabled == 1 || g_iCvar_AutoShove_Enabled == 2 && !FVectorInViewAngle(iClient, fInfectedPos)) && fInfectedDist <= 6400.0 && !L4D_IsPlayerIncapacitated(iClient)
			...
			&& (!SurvivorHasMeleeWeapon(iClient) || iCurWeapon != iWpnSlots[1]))
		{
			...
			if (iInfectedClass != L4D2ZombieClass_Boomer || !bFriendIsNearBoomer && fTargetDist > BOT_BOOMER_AVOID_RADIUS_SQR)
			{
				if (iCurWeapon == iWpnSlots[0])
				{
					...
					if (SurvivorHasShotgun(iClient))
					{
						...
						if (fTargetDist <= (g_fCvar_TargetSelection_ShootRange4*g_fCvar_TargetSelection_ShootRange4) && fTargetDist > ((fShootRange * 1.1)*(fShootRange * 1.1))
							&& GetGameTime() > g_fSurvivorBot_NextWeaponRangeSwitchTime[iClient] && !IsWeaponReloading(iCurWeapon, false) && !SurvivorHasMeleeWeapon(iClient))
						{
				...
				else if (iCurWeapon == iWpnSlots[1] && !SurvivorHasMeleeWeapon(iClient))
				{
					...
					if (fTargetDist <= (fShotgunRange*fShotgunRange) && GetGameTime() > g_fSurvivorBot_NextWeaponRangeSwitchTime[iClient]
						&& GetClientPrimaryAmmo(iClient) > 0 && !IsWeaponReloading(iCurWeapon) && SurvivorHasShotgun(iClient))
					{
						g_fSurvivorBot_NextWeaponRangeSwitchTime[iClient] = GetGameTime() + GetRandomFloat(1.0, 3.0);
						g_bSurvivorBot_ForceSwitchWeapon[iClient] = true;
						SwitchWeaponSlot(iClient, 0);
			...
			if (!IsSurvivorBotBlindedByVomit(iClient) && (bHoldKey || !IsSurvivorBusy(iClient, true, true, true)))
			{
				...
				else
				{
					...
					if ( bAllowScavenge && bCanRegroup && fDistanceToRegroup < fMaxDist
						...
						(!IsValidClient(iTankTarget) || GetClientDistance(iClient, iTankTarget, true) > 262144.0
						&& GetVectorDistance(g_fClientAbsOrigin[iTankTarget], fScavengePos, true) > 147456.0) )
					{
...
Action OnSurvivorSwitchWeapon(int iClient, int iWeapon) 
{
	...
	else if (iWeapon == GetClientWeaponInventory(iClient, 1)) 
	{
		if (iCurWeapon == GetClientWeaponInventory(iClient, 0) && GetClientPrimaryAmmo(iClient) > 0)
		{
			...
			if (GetSurvivorBotWeaponPreference(iClient) != L4D_WEAPON_PREFERENCE_SECONDARY && (!SurvivorHasMeleeWeapon(iClient)
				|| g_iSurvivorBot_NearbyInfectedCount[iClient] < g_iCvar_ImprovedMelee_SwitchCount )
				&& ((g_bCvar_DontSwitchToPistol && SurvivorHasSniperRifle(iClient)) || SurvivorHasShotgun(iClient)))
			{
				return Plugin_Handled;
			}
		}
...
bool SurvivorBot_IsTargetShootable(int iClient, int iTarget, int iCurWeapon, float fAimPos[3])
{
	...
	if (!bInViewCone)
	{
		...
		if (iCurWeapon == iPrimarySlot && SurvivorHasShotgun(iClient))
			fCone *= 2.0;
	...
	if (IsValidClient(iTarget))
	{
		...
		if (GetClientTeam(iTarget) == 2 && !L4D_IsPlayerPinned(iTarget) && !L4D_IsPlayerIncapacitated(iClient))
		{
			if (bInViewCone && !g_bCvar_BotsShootThrough && (iCurWeapon == iPrimarySlot || iCurWeapon == GetClientWeaponInventory(iClient, 1)
				&& (!SurvivorHasMeleeWeapon(iClient) || GetClientDistance(iClient, iTarget) <= 96.0)))
				return false;
			...
			if (g_bCvar_BotsFriendlyFire) 
			{
				...
				&& SurvivorHasTier3Weapon(iClient) == 1 && GetVectorVisible(fAimPos, g_fClientCenteroid[iTarget]))
					return false;
			}
		}
...
{	
	...
	if (iCurWeapon == GetClientWeaponInventory(iClient, 0))
	{
		...
		&& SurvivorHasTier3Weapon(iClient) == 1 && GetVectorVisible(fAimPos, g_fClientCenteroid[iClient]))
		{
			if (IsEntityExists(g_iSurvivorBot_TargetInfected[iClient]) && (!SurvivorHasMeleeWeapon(iClient)
...
void GetTargetAimPart(int iClient, int iTarget, float fAimPos[3])
{
	...
	if (IsWitch(iTarget) && fDist <= 65536.0 && IsWeaponSlotActive(iClient, 0) && SurvivorHasShotgun(iClient) || (L4D_IsPlayerIncapacitated(iClient)
...
int CalculateGrenadeThrowInfectedCount()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		iFreeSurvivors++;
		if ( IsWeaponSlotActive(i, 1) && SurvivorHasMeleeWeapon(i) == 2)
			iFreeSurvivors++;
	}
...
public Action L4D2_OnFindScavengeItem(int iClient, int &iItem)
{
	...
	if (IsEntityExists(iSecondarySlot))
	{
		...
		if ( iItemFlags & FLAG_MELEE && (!SurvivorHasMeleeWeapon(iClient) && ( GetSurvivorTeamInventoryCount(FLAG_MELEE) >= g_iCvar_MaxMeleeSurvivors )
		...
		if ( iItemFlags & (FLAG_PISTOL | FLAG_PISTOL_EXTRA) && SurvivorHasMeleeWeapon(iClient) == 2
	...
	if (IsWeaponSlotActive(iClient, 3) && iItemFlags & (FLAG_MEDKIT | FLAG_DEFIB) && SurvivorHasHealthKit(iClient) == 3)
		return Plugin_Handled;

	if (iItemFlags & FLAG_MEDKIT && IsEntityExists(g_iSurvivorBot_DefibTarget[iClient]) && SurvivorHasHealthKit(iClient) == 2)
		return Plugin_Handled;
...
float GetEntityTravelDistance(int iClient, int iEntity, bool bSquared = false)
{
	...
	static float fEntityPos[3], fTargetPos[3];
